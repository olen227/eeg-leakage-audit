...
1. X_raw.npy — массив окон, форма (N, C, L), тип float32, формат .npy (читается memmap).
   ...
   Допускается float16 для наборов, чей кэш в float32 не помещается на диск (TUSZ:
   ~90 ГБ против 45). Потребители приводят батчи к float32 при чтении; относительная
   точность float16 около 0,05 %, что на порядки ниже шума ЭЭГ. Единицы хранения
   при этом обязаны быть такими, чтобы значения не попадали в субнормальный диапазон
   float16 (< 6·10⁻⁵): сигнал в вольтах туда попадает, в микровольтах — нет. Масштаб
   на измерение не влияет — окна нормируются пофолдово по обучающей части.
...
def check_dataset(cache_dir, identity_map=None):
    ...
    if X.dtype == np.float16:
        warnings.append("X_raw.npy хранится в float16 (допустимо по контракту при "
                        "нехватке диска); потребители приводят батчи к float32")
